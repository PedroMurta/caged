...
def baixar_arquivo(ftp, ano_mes):
    ano = ano_mes[:4]
    filename = f"CAGEDMOV{ano_mes}.7z"
    ...
    try:
        ftp.cwd(f"{ftp_base_path}/NOVO CAGED/{ano}/{ano_mes}")
...
def extrair_arquivo(caminho_7z, ano_mes):
    ...
    caminho_txt = os.path.join(pasta_destino, f"CAGEDMOV{ano_mes}.txt")
...
def carregar_e_processar(spark, caminho_txt):
    try:
        df = spark.read.csv(
            caminho_txt, sep=';', header=True, inferSchema=True, encoding='latin1'
        )
...
# ========================== EXECUÇÃO ==========================
os.makedirs(raw_7z_path, exist_ok=True)
...
try:
    ...
    iceberg_meses = spark.sql(f"SELECT DISTINCT competencia FROM {iceberg_table_name}").rdd.flatMap(lambda x: x).collect()
    ...
    for ano_mes in tqdm(meses_para_processar, desc="🔄 Processando meses", unit="mês"):
        ...
        if df and salvar_em_parquet(df, ano_mes):
            salvar_no_iceberg(df, ano_mes)
            tqdm.write(f"📦 Arquivos mantidos em raw/ para {ano_mes}")
